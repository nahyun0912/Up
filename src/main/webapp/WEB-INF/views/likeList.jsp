      ...
      <div class="board_list_wrap">
         <div class="board_list">
            ...
            <c:forEach var="bb" items="${book }">
               <div>
                  <div class="num">지하철</div>
                  <div class="title">
                     <a href="map.view.go"> ${bb.sub_station }${bb.sub_line_name }</a>
                  </div>
                  <div class="count"><a href="map.deleteMyBk?u_id=${loginMember.u_id  }&sub_no=${bb.sub_no }">즐겨찾기 삭제</a></div>
               </div>
            </c:forEach>
            <c:forEach var="ff" items="${book2 }">
               <div>
                  <div class="num">공공시설</div>
                  <div class="title">
                     <a href="facMap.View.go"> ${ff.f_fclt_nm }</a>
                  </div>
                  <div class="count"><a href="map.deleteMyBk?u_id=${loginMember.u_id  }&f_no=${ff.f_no }">즐겨찾기 삭제</a></div>
               </div>
            </c:forEach>
            <c:forEach var="cc" items="${book3 }">
               <div>
                  <div class="num">휠체어충전소</div>
                  <div class="title">
                     <a href="chargeMap.View.go"> ${cc.ch_FCLTYNM }</a>
                  </div>
                  <div class="count"><a href="map.deleteMyBk?u_id=${loginMember.u_id  }&ch_no=${cc.ch_no }">즐겨찾기 삭제</a></div>
               </div>
            </c:forEach>
         </div>
         <div>
